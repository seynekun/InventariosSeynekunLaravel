    ...
    <x-wire-card>
        <form wire:submit='save' class="space-y-4">
            <div class="grid gap-4 lg:grid-cols-4">
                ...
                <x-wire-select label='Cotización' wire:model.live='quote_id' :async-data="[
                    'api' => route('api.sales.index'),
                    'method' => 'POST',
                ]" option-label="name"
                    option-value="id" placeholder="Selecciona una cotización" option-description="description" />

                <div class="col-span-2">
                    <x-wire-select label='Cliente' wire:model='customer_id' :async-data="[
                        'api' => route('api.customers.index'),
                        'method' => 'POST',
                    ]" option-label="name"
                        option-value="id" placeholder="Selecciona un cliente" />
                </div>
                <div class="col-span-2">
